	...
	<view>
		...
		<view class="scroll">
			<scroll-view :scroll-into-view="to" scroll-y style="width: 100%;height: 100%;">
				<view class="info" v-for="(item,index) in dep" @click="toDetails()">
					<view v-show="isCheckbox">
						<u-checkbox size="15" shape="circle" />
					</view>
					<view class="icon">
						<view :style="{'background':'#3196fa'}" class="icon">
							{{item['name'].slice(0,1)}}
						</view>
					</view>
					<view class="item">
						<text class="name" v-text="item.name"></text>
						<text class="post" v-text="item.departmentName"></text>
					</view>
				</view>
			</scroll-view>
		</view>
...
<script>
	import {
		dep
	} from '../../pages/contacts/config/data.js'
