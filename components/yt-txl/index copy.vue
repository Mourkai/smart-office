<template>
	<view class="yt-txl-container">
		<view class="search">
			<view class="close" v-show="isCheckbox">
				<u-button type="primary">确定</u-button>
				<u-button @click="close">关闭</u-button>
			</view>
			<input type="text" @input="inputFunc" class="s-input" placeholder="搜索" />
			<view class="title" v-show="!isCheckbox">
				<view class="dep" @click="toPage()" v-text="orgName"></view>
				<view class="icon">
					<u-icon name="arrow-right" size="15"></u-icon>
				</view>
			</view>
		</view>
		<u-checkbox-group v-model="checkboxValue1" placement="column" @change="checkboxChange">
			<view class="scroll" :style="index ? '' :'right:25upx'">
				<scroll-view :scroll-into-view="to" scroll-y style="width: 100%;height: 100%;">
					<view :id="o.key" v-for="(o,i) in resource" :key="i">
						<view class="p">{{o.key}}</view>
						<view @click="clickFunc(item)" v-for="(item,index) in o.data" class="info" :key="index">
							<view v-show="isCheckbox">
								<u-checkbox size="15" shape="circle" />
							</view>
							<view :style="{'background':item.color}" v-show="!item['avatar']" class="icon">
								{{item[name].slice(0,1)}}
							</view>
							<view v-show="item['avatar']" class="icon">
								<u--image :src="item['avatar']" shape="circle" width="40px" height="40px"></u--image>
							</view>
							<view class="item">
								<text class="name">{{item[name]}}</text>
								<text class="post">{{item.orgName}}</text>
							</view>
						</view>
					</view>
					<view class="count">
						共{{count}}位联系人
					</view>
				</scroll-view>
			</view>
		</u-checkbox-group>
		<view class="flag" v-if="index">
			<scroll-view scroll-y="true" :show-scrollbar="false" class="flag-scroll" style="width: 100%;height: 100%;">
				<view @click="toFunc(o.key)" class="flag-key" v-for="(o,i) in resource" :key="i">
					{{o.key}}
				</view>
			</scroll-view>
		</view>
	</view>
</template>

<script>
	'use strict';
	import localCache from '@/util/cache.js'
	let pinyin = new(require('./pinyin'))({
		charCase: 0
	});
	import {
		getAppUserList
	} from '@/api/system.js'
	export default {
		props: {
			name: {
				type: String,
				default () {
					return "name";
				}
			},
			isCheckbox: {
				type: Boolean,
				default: false
			},
			index: {
				type: Boolean,
				default: true
			}
		},
		data() {
			return {
				//{"key":"A","data":[{"id":"","name":"1213","orgId":"107","orgName":"111","color":"#3c9cff"}]}
				resource: [],
				chars: [],
				to: "",
				data: [],
				orgName: '',
				count: 0,
				cache: [],
				checkboxValue1: []
			};
		},
		beforeMount() {
			uni.showLoading({
				title: '加载中'
			});
		},
		async mounted() {
			const userInfo = localCache.getCache('userInfo')
			this.orgName = `${userInfo.orgParentName}>${userInfo.orgName}`
			const contacts = localCache.getCache('contacts')
			if (contacts) {
				this.data = contacts
			} else {
				const result = await getAppUserList()
				const list = ['#3c9cff', '#f9ae3d', '#5ac725', '#f56c6c'];
				this.data = result.data.map(item => {
					item.color = list[Math.floor(Math.random() * list.length)]
					return item
				})
				localCache.setCache('contacts', this.data)
			}
			this.count = this.data.length
			this._parseData(this.data)
		},
		methods: {
			toFunc(o) {
				this.to = o
			},
			close() {
				this.$emit('close')
			},
			clickFunc(item) {
				if (!this.isCheckbox) {
					uni.navigateTo({
						url: '/pages/contacts/details?userId=' + item.id + '&color=' + item.color
					})
				}
			},
			checkboxChange() {},
			inputFunc(r) { //搜索功能
				if (!r.detail.value) {
					this.resource = this.cache
					return
				}

				let temp = []
				this.cache.forEach(o => {
					o.data.forEach(item => {
						if (item[this.name].indexOf(r.detail.value) > -1) { // 匹配到
							// 确定当前这个元素的key是谁
							let key = o.key
							// 找到temp中的key
							let index = 0 //下标
							let find = false // 数据是否存在
							for (let d in temp) {
								if (temp[d].key === o.key) {
									index = d
									find = true
								}
							}
							if (find) { // 如果key已经存在，直接插入数据
								temp[index].data.push(item)
							} else { // 不存在初始化一个并存入
								temp.push({
									key: o.key,
									data: [item]
								})
							}
						}
					})
				})
				this.count = temp.length
				this.resource = temp
			},
			toPage() {
				uni.navigateTo({
					url: '/pages/contacts/dep'
				})
			},
			_type(val) {
				return Object.prototype.toString.call(val).slice(8, -1).toLowerCase()
			},
			_parseData(r) {
				const contactsCache = localCache.getCache('contactsCache')
				if (contactsCache) {
					//预先加载十个
					let count = 0;
					contactsCache.map((item, index) => {
						if (count < 10) {
							this.resource.push(item)
							this.cache.push(item)
							count += item.data.length
						}
					})
					setTimeout(() => {
						this.resource = contactsCache
						this.cache = contactsCache
					}, 1000)
					//this.cache = contactsCache
				} else {
					// 生成a-z的数组
					let data = [];
					this.chars = []
					for (let i = 65; i <= 90; i++) {
						let key = String.fromCharCode(i)
						data.push({
							"key": key,
							data: []
						})
						this.chars.push(key)
					}
					if (this._type(r) === "array") {

						// 填充数据
						r.forEach(o => {
							// 找到char的位置
							data.forEach((item, index) => {
								let a = this._parseChar(o[this.name || 'name'])
								if (item.key === a) {
									data[index].data.push(o)
								}
							})
						})
					}
					// 组合最后数据并踢出没有匹配到a-z中的任意数据
					let finalData = []
					for (let i in data) {
						if (data[i].data.length > 0) {
							finalData.push(data[i])
						}
					}
					this.resource = finalData
					this.cache = finalData
					localCache.setCache('contactsCache', finalData)
					finalData = null
				}
			},
			_parseChar(name) {
				if (Object.prototype.toString.call(name).slice(8, -1) !== 'String') {
					console.error("name is not string")
					return
				}
				let chars = pinyin.getFullChars(name);
				return chars[0].toUpperCase()
			}
		}
	};
</script>

<style lang="scss" scoped>
	page {
		background: #f4f4f4;
	}

	.search {
		width: 100%;
		display: flex;
		justify-content: center;
		flex-direction: column;
		align-items: center;
		background-color: #fff;
		padding-top: 30rpx;

		.close {
			display: flex;
			width: 100%;
			height: 80rpx;
			justify-content: space-around;

			.u-button {
				width: 30%;
				height: 60rpx;
			}
		}

		.title {
			display: flex;
			justify-content: center;
			align-items: center;
			text-align: left;
			width: 100%;
			padding: 10rpx;
			overflow: hidden;
			font-size: 30rpx;

			.dep {
				color: #666;
				width: 90%;
				padding-left: 30rpx;
				overflow: hidden;
				white-space: nowrap;
				text-overflow: ellipsis;
				-o-text-overflow: ellipsis;
			}

			.icon {
				width: 10%;
			}
		}
	}

	.s-input {
		width: 90%;
		height: 75rpx;
		background: url('data:image/png;base64,') 20upx center no-repeat #fff;
		background-size: 40upx;
		text-indent: 80upx;
		border-radius: 40rpx;
		border: 1px solid $main-color;
		font-size: 30rpx;
	}

	.scroll {
		position: absolute;
		left: 0;
		top: 210rpx;
		right: 0;
		bottom: 0;
	}

	.p {
		top: 0;
		left: 0;
		background: #f4f4f4;
		font-size: 28upx;
		margin-bottom: 10upx;
		text-indent: 40upx;
		font-weight: bold;
	}

	.info {
		display: flex;
		justify-content: flex-start;
		align-items: center;
		margin-bottom: 4rpx;
		padding: 20upx 25upx;
		color: #444444;
		font-size: 32rpx;
		background: #fff;

		.post {
			font-size: 30rpx;
			color: #666666;
		}
	}

	.icon {
		width: 80upx;
		height: 80upx;
		border-radius: 50%;
		margin-right: 20upx;
		display: flex;
		justify-content: center;
		align-items: center;
		font-size: 30upx;
		color: #fff;
	}

	.item {
		height: 100upx;
		display: flex;
		flex-direction: column;
		justify-content: space-around;
	}

	.flag {
		width: 50upx;
		position: fixed;
		top: 350rpx;
		right: 20rpx;
		bottom: 25upx;
	}

	.flag-scroll {
		padding-top: 10upx;
	}

	.flag-key {
		padding: 0;
		margin: 0 auto 10upx auto;
		width: 30upx;
		height: 30upx;
		color: $main-color;
		display: flex;
		justify-content: center;
		align-items: center;
		font-size: 28rpx;
	}

	.count {
		text-align: center;
		padding: 50rpx 0 100rpx 0;
		background-color: #fff;
		font-size: 30rpx;
		color: #666;
	}
</style>
