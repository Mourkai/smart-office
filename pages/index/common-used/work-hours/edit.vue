<template>
	<view class="add bp50">
		<view class="header">
			...
			<view class="desc">
				{{resetData.fillStartDate}} ~ {{resetData.fillEndDate}}
			</view>
		</view>
		...
		<view class="btn-group" v-show="showBlock">
			<u-button type="info" shape="circle" text="重置" @click="reset"></u-button>
...
<script>
	...
	export default {
		...
		methods: {
			reset(){
				const objStr = JSON.stringify(this.resetData)
				const objParse = JSON.parse(objStr)
				this.forData(objParse)
			},
			async forData(data){
				const weekNum = ['一', '二', '三', '四', '五', '六', '日']
				const day7 = [];
				const dayData = data.workingHourRecordDtoList
				let currentDate = new Date(data.fillStartDate);
				...
				for (let i = 0; i < 7; i++) {
					...
					if (dayData.length > 0) {
						...
						dataItem.forEach(item => {
							...
							item._workType = ''
							this.workType[0].forEach((item2, index) => {
			...
			async submit() {
				...
				const formData = {
					fillEndDate: this.resetData.fillEndDate,
					fillStartDate: this.resetData.fillEndDate,
					id: this.id,
