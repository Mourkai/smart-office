	...
	<view>
		<view class="content">
			...
			<view class="content-item">
				...
				<view class="desc" v-text="data.noticeAwardDto && data.noticeAwardDto.noticeCode"></view>
			</view>
			<view class="content-item">
...
<script>
	...
	export default {
		...
		async mounted() {
			let typeList = await getContractType()
			this.typeList = typeList.data
			let {data} = await getSignContractDetail(this.id)
			const code = await listAndNoticeCode()
			const codeData = code.data.rows.find(item=>item.id === data.biddingId)
			if(codeData){
				data.contractCode = codeData.code
			}
			this.data = data
		} 
	}
