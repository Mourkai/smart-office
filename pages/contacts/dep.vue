<template>
	<view>
		<scroll-view :scroll-into-view="to" scroll-y style="width: 100%;height: 100%;">
			<view class="info" v-for="(item,index) in tmp" @click="toDetails(item.id,item.color)">
				<view class="icon">
					<Avatar :sex="item['sex']" size="100rpx" />
				</view>
				<view class="item">
			...
			<u-empty v-if="tmp.length==0" icon="/static/public/no-data.png" />
		</scroll-view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		async onLoad({orgId}) {
			...
			const {data} = await getAppUserList({orgId})
			this.dep = data.rows.map(item => {
				if(!item.name){
					item.name = '暂无'
				}
				return {
		...
		methods: {
			...
			search() {
				this.tmp = this.dep.filter(item => item.name.indexOf(this.keyword) != -1)
			}
		},
	};
</script>
